...
- **Advertising**: Carousel banner ads with tiered pricing and an in-browser ad designer. Placeholder ad slots in the "Featured Local Businesses" carousel are clickable and route to `/advertising` (the purchase landing page) so empty spots actively generate leads instead of being dead clicks; real paid ads (id>0) instead open a preview modal that links to the business listing or website.
