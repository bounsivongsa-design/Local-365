...
- Auto-sync now exists: the dev server pushes main to GitHub hourly (server/githubSync.ts, no-op when up to date) and emails admins on failure (24h rate limit). Skipped in deployments (no git checkout) and when GITHUB_TOKEN is unset. Note: repo is public, so ls-remote succeeds even with a bad token — auth errors only surface on actual push.
